...
GanonInput = open(sys.argv[4])
GanonInput_uncl = open(sys.argv[5])
resultfile = open(sys.argv[6], 'w')
...
seenInputID = ""
for entry in GanonInput:
	entry = entry.rstrip("\r\n")
	if entry == "":
		break
	entry = entry.split("\t")
	idString = (entry[0].split(" "))[0]
	id_raw = (idString).split(";")
	name = id_raw[0]
	if id_raw[1] == seenInputID:
		continue
	seenInputID = id_raw[1]
...
for entry in GanonInput_uncl:
	entry = entry.rstrip("\r\n")
	if entry == "":
		break
	idString = (entry.split(" "))[0]
	id_raw = (idString).split(";")
	name = id_raw[0]
	origTax = accToTax[name] if name in accToTax else ""
	numberOfReads += 1
